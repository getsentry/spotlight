...
**What the AI does:**
1. Calls `errors_search` with recent timeWindow
...
AI workflow:
1. traces_search → Find checkout traces
2. spotlight.traces.get → Analyze slow trace
3. logs_search → Check related logs
4. errors_search → Look for errors
```
...
[Calls logs_search for recent activity]
