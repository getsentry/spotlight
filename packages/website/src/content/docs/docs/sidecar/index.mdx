...
- **This machine's IPs**: Any hostname that resolves to an IP assigned to this machine, including:
  ...
  - Local network IPs (e.g., `192.168.x.x`, `10.x.x.x`)
  - VPN IPs (e.g., Tailscale, Zerotier)
- **spotlightjs.com**: The official Spotlight website (HTTPS only)

### Custom Local Hostnames

You can use custom hostnames in your `/etc/hosts` file (e.g., `myapp.local` pointing to `127.0.0.1`). The sidecar resolves hostnames via DNS and allows them if they resolve to any of the machine's IP addresses.
...
The sidecar implements protection against [DNS rebinding attacks](https://en.wikipedia.org/wiki/DNS_rebinding), which could potentially allow a malicious website to access your local sidecar.

**How DNS rebinding works:**
1. An attacker controls `evil.com` with a short DNS TTL
2. User visits `evil.com`, which initially resolves to the attacker's server
3. The attacker's page loads malicious JavaScript
4. The attacker rebinds `evil.com` to `127.0.0.1` (or another machine IP)
5. JavaScript makes requests to `evil.com` (now pointing to the machine)

**Our mitigation:**
- DNS resolution results are cached for a **minimum of 6 hours**
- This means an attacker would need to:
  - Trick the user into visiting their malicious site
  - Keep the user on the page (or have them revisit) for 6+ hours
  - Then rebind the DNS to the machine's IP
- Even then, rebinding to a local IP would make the attacker's site inaccessible for the cache duration

This makes DNS rebinding attacks impractical against Spotlight. The attacker would need to actively mislead users into visiting their site, wait at least 6 hours hoping users revisit (a revisit won't work if DNS is already rebound), and then rebind the DNS record to a local IP. Even in that scenario, using a low TTL for the local IP record would cause their site to be down for 6 hours.
