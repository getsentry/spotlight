...
- **Localhost origins**: Any hostname that resolves to `127.0.0.0/8` (IPv4) or `::1` (IPv6)
- **spotlightjs.com**: The official Spotlight website (HTTPS only)

### Custom Local Hostnames

You can use custom hostnames in your `/etc/hosts` file (e.g., `myapp.local` pointing to `127.0.0.1`). The sidecar resolves hostnames via DNS and allows them if they resolve to localhost addresses.
...
4. The attacker rebinds `evil.com` to `127.0.0.1`
5. JavaScript makes requests to `evil.com` (now pointing to localhost)
...
- Even then, rebinding to `127.0.0.1` would make the attacker's site inaccessible for the cache duration

This makes DNS rebinding attacks impractical against Spotlight. The attacker would need to actively mislead users into visiting their site, wait at least 6 hours hoping users revisit (a revisit won't work if DNS is already rebound), and then rebind the DNS record to `127.0.0.1`. Even in that scenario, using a low TTL for the `127.0.0.1` record would cause their site to be down for 6 hours.
