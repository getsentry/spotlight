  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        // We have to use the `await import()` syntax here to prevent Astro pulling these
        // into the client side. Don't ask me why it does this but be warned!
        const { default: spotlight, buildClientInit } = await import('@spotlightjs/spotlight/vite-plugin');
        ...
        const [path, url] = await Promise.all([import('path'), import('url')]);
