...
import type { Envelope, EnvelopeItem, RawEventContext } from "./nodeAdapter.js";

// Local utility to avoid circular dependency with overlay package
...
export function processEnvelopeNodeJs(rawEvent: RawEventContext) {
  ...
  while (buffer.length) {
    ...
    try {
      ...
      // data sanitization
      if (itemHeader.type) {
        // We should really stop adding type to payloads
        (itemPayload as any).type = itemHeader.type;
      }
    } catch (err) {
