...
export function createSpotlightMcpServer(eventProcessor: McpEventProcessor) {
  ...
  server.registerTool(
    ...
    async ({ count = 10, level, traceId }) => {
      ...
      const enrichedErrors = events.map((event: any) => ({
        id: event.event_id,
        message: event.exception?.values?.[0]?.value,
        type: event.exception?.values?.[0]?.type,
        stackTrace: event.exception?.values?.[0]?.stacktrace?.frames?.map((frame: any) => ({
  ...
  server.registerTool(
    ...
    async ({ traceId }) => {
      ...
      const relatedEvents = eventProcessor
        .getEvents()
        .filter((event: any) => event.contexts?.trace?.trace_id === traceId);
            ...
            text: JSON.stringify(
              {
                ...
                span_tree: trace.spanTree.map((span: any) => ({
                ...
                performance_issues: trace.spanTree
                  .filter((span: any) => span.timestamp - span.start_timestamp > 1000)
                  .map((span: any) => ({
                    span_id: span.span_id,
                    description: span.description,
                    duration_ms: span.timestamp - span.start_timestamp,
            ...
            text: JSON.stringify(
              {
                ...
                related_trace: trace
                  ...
                  : null,
                correlated_logs: logs.map((log: any) => ({
                  id: log.id,
                ...
                debugging_suggestions: [
                  ...
                  error.breadcrumbs?.length
                    ? `${error.breadcrumbs.length} breadcrumbs available showing user journey`
                    : null,
                ].filter(Boolean),
              },
              null,
  ...
  server.registerTool(
    ...
    async ({ limit = 20 }) => {
      ...
      const traceSummaries = traces.map((trace: any) => ({
        trace_id: trace.trace_id,
        root_transaction: trace.rootTransactionName,
        status: trace.status,
        span_count: trace.spans.size,
...
${stackFrames
  .slice(-5)
  .map((frame: any) => `  at ${frame.function || "<anonymous>"} (${frame.filename}:${frame.lineno}:${frame.colno})`)
  .join("\n")}
...
${
  error.breadcrumbs?.length
    ? `\nRecent User Actions (${error.breadcrumbs.length} breadcrumbs available):\n${error.breadcrumbs
        .slice(-3)
        .map((b: any) => `- ${b.message || b.category}`)
        .join("\n")}`
    : ""
}
...
4. Suggested fixes or improvements`,
  ...
  server.registerResource(
    ...
    async () => {
      ...
      const enrichedErrors = errors.map((event: any) => ({
        id: event.event_id,
        message: event.exception?.values?.[0]?.value,
        type: event.exception?.values?.[0]?.type,
        timestamp: event.timestamp,
