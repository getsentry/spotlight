...
export class McpIntegration {
  private eventProcessor: McpEventProcessor;
  private mcpServer: ReturnType<typeof createSpotlightMcpServer> | null = null;
  private transport: StreamableHTTPServerTransport | null = null;
  private options: McpIntegrationOptions;

  constructor(options: McpIntegrationOptions = {}, contextLinesHandler?: ContextLinesHandler) {
    this.options = { enabled: true, ...options };
    this.eventProcessor = new McpEventProcessor(contextLinesHandler);
   ...
   */
  getEventProcessor(): McpEventProcessor {
  ...
  subscribeToBuffer(buffer: MessageBuffer<Payload>): () => void {
    ...
    const subscription = buffer.subscribe(([contentType, data]: [string, Buffer]) => {
