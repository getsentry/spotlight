...
import type { SentryTransactionEvent } from "@spotlightjs/core/sentry";
import { handleProcessEnvelope } from ".";
import useSentryStore from "./store";

describe("Sentry Integration", () => {
  test("Process Envelope Empty", () => {
    const envelope = fs.readFileSync("./_fixtures/envelope_empty.txt");
    expect(handleProcessEnvelope({ data: envelope, contentType: "test" })).not.toBe(undefined);
  });
  ...
  test("Process Astro SSR pageload (BE -> FE) trace", () => {
    ...
    const processedNodeEnvelope = handleProcessEnvelope({ data: nodeEnvelope, contentType: "test" });

    const browserEnvelope = fs.readFileSync("./_fixtures/envelope_astro_ssr_browser.txt");
    const processedBrowserEnvelope = handleProcessEnvelope({ data: browserEnvelope, contentType: "test" });
