...
export default function initTelemetry(options: TelemetryOptions = {}) {
  return {
    name: "sentry",
    forwardedContentType: [HEADER],

    setup: ({ navigate, sidecarUrl }: { navigate: (path?: string) => void; sidecarUrl: string }) => {
      ...
      log("Setting up telemetry for Spotlight");
      ...
      if (options.openLastError) {
        store.subscribe("event", (e: SentryEvent) => {
          ...
          setTimeout(() => navigate(`/errors/${e.event_id}`), 0);
        });
      }
      ...
      const onRenderError = (e: CustomEvent) => {
        ...
        store.pushEvent(e.detail.event).then(() => navigate(`/errors/${e.detail.event.event_id}`));
      };
 ...
 */
function addSpotlightIntegrationToSentry(options: TelemetryOptions, sidecarUrl?: string) {
