...
export default function TelemetrySidebar({ errorCount, traceCount, logCount, isOnline }: TelemetrySidebarProps) {
  ...
  const [isInsightsExpanded, setIsInsightsExpanded] = useState(isInsightsActive);

  useEffect(() => {
    if (isInsightsActive && !isInsightsExpanded) {
      setIsInsightsExpanded(true);
    }
  }, [isInsightsActive, isInsightsExpanded]);
  ...
  return (
    <nav
      className="flex shrink-0 flex-col border-r border-primary-700"
      style={{ width: "240px" }}
